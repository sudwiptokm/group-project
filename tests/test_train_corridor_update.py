"""Fast, SUMO-free guard that a PPO update MOVES the shared policy -- for both
the IPPO (local-obs critic) and MAPPO (joint-state critic) paths. Catches a
broken gradient flow / no-op optimizer without needing SUMO.
"""
import copy
...
def _buffer(state_dim, n=32):
    """Per-agent buffer with both `obs` (actor input, OBS_DIM) and `state`
    (critic input, state_dim). Reward correlates with action so there is a
    real gradient signal to fit."""
    per = {}
    for agent in ("C1", "C2"):
