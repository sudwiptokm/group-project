...
class SafetyLoggingEnv(SumoEnvironment):
    ...
    def _get_safety_info(self) -> dict:
        ...
        # _safety_components() call at the action step. The corridor branch read
        # the instantaneous sample per signal, which is the defect the window
        # exists to fix: exposure can never fire there (is_yellow is always down
        # by the time the action step is reached) and braking is undersampled by
        # a factor of delta_time. The window already accumulates per signal, so
        # the per-agent breakdown the corridor needs costs nothing extra.
        ...
        for tls_id in self.traffic_signals:
            b, e = self._safety_window.for_ts(tls_id)
            info[f"safety_total_{tls_id}"] = b + e
        return info
...
def make_corridor_env(seed: int, scenario: str = "corridor_offpeak",
    ...
    `min_green`, `teleport` and `tripinfo` mean exactly what they mean in
    make_env, and are resolved the same way -- see that docstring. This factory
    used to hard-wire min_green=10, which is the floor analysis/actuated.py
    measured a perfect-information controller to be 5.6x worse than a fixed
    plan at. Every single-intersection peak run was conducted there, and that
    is the defect docs/FINDINGS_2026-08-12.md exists to document; a corridor
    run inheriting the same hard-wired floor would repeat it on three signals
    instead of one.

    The corridor floor is not assumed to be the single junction's 60 s. It is a
