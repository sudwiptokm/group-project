"""SP13b follow-up: with span=400 (1 crossing) and span=700 (3 crossings)
sampled (docs/FINDINGS_2026-08-26-sp13-geometry-dose-response.md,
docs/FINDINGS_2026-08-27-sp13b-span700-confound.md), does a third span point
midway between them move monotonically toward span=700's shape, or something
else?

Same 8 ratio points as analysis/geometry_sweep.py / geometry_sweep_span700.py
(r = 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.90), same corridor_peak/
min_green=10 zero-shot protocol, same seed sets -- only the net files differ
(analysis/build_geometry_sweep_nets_span550.py's span=550 builds, all new;
none of the span=400 or span=700 sweeps' nets are reusable here since C2's
absolute position differs for every r at a different span).

Deliberately bypasses compare.py -- same reason geometry_sweep.py gives.

    python -m analysis.build_geometry_sweep_nets_span550   # once
    python -m analysis.geometry_sweep_span550
"""
import os
import sys
...
OUT_CSV = os.path.join(REPO, "analysis", "geometry_sweep_span550.csv")
...
RATIO_NETS = {
    0.50: "corridor_geom550_275.net.xml",
    0.55: "corridor_geom550_302.net.xml",
    0.60: "corridor_geom550_330.net.xml",
    0.65: "corridor_geom550_358.net.xml",
    0.70: "corridor_geom550_385.net.xml",
    0.75: "corridor_geom550_412.net.xml",
    0.80: "corridor_geom550_440.net.xml",
    0.90: "corridor_geom550_495.net.xml",
}
...
def report(df: pd.DataFrame) -> None:
    print(f"\n=== {SCENARIO}, mg{MIN_GREEN}, span=550: delay/trip vs ratio ===")
