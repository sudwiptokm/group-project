"""SP13 follow-up: every doc in this series (SP13/SP13b/SP13c/SP13d) swept r
in [0.50, 0.90] at span=400/450/550/700 -- the short block always SECOND
(C1-C2 longer than C2-C3). r<0.50 (short block FIRST) was flagged as
untested in every one of those docs. This fills that gap at span=400 (SP13's
original axis; the other three spans' low-r arms are still open).

Same 7 new ratio points as build_geometry_sweep_nets_lowr.py (r = 0.45,
0.40, 0.35, 0.30, 0.25, 0.20, 0.10), plus r=0.50 (corridor.net.xml, reusing
geometry_sweep.py's own cached tripinfo rather than re-running it -- same
stem convention). Same corridor_peak/min_green=10 zero-shot protocol, same
seed sets as every other sweep in this series.

Deliberately bypasses compare.py -- same reason geometry_sweep.py gives.

    python -m analysis.build_geometry_sweep_nets_lowr   # once
...
OUT_CSV = os.path.join(REPO, "analysis", "geometry_sweep_lowr.csv")
...
RATIO_NETS = {
    0.10: "corridor_geomlo40.net.xml",
    0.20: "corridor_geomlo80.net.xml",
    0.25: "corridor_geomlo100.net.xml",
    0.30: "corridor_geomlo120.net.xml",
    0.35: "corridor_geomlo140.net.xml",
    0.40: "corridor_geomlo160.net.xml",
    0.45: "corridor_geomlo180.net.xml",
...
def report(df: pd.DataFrame) -> None:
    print(f"\n=== {SCENARIO}, mg{MIN_GREEN}, span=400, r<=0.50: delay/trip vs ratio ===")
