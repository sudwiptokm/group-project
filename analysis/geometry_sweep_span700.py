"""SP13 follow-up: does the [r~0.51, r~0.80] green_wave failure band (found on
a span=400 axis, docs/FINDINGS_2026-08-26-sp13-geometry-dose-response.md)
hold at the same ratios on a span=700 axis, or do its boundaries shift?

Same 8 ratio points as analysis/geometry_sweep.py (r = 0.50, 0.55, 0.60, 0.65,
0.70, 0.75, 0.80, 0.90), same corridor_peak/min_green=10 zero-shot protocol,
same seed sets -- only the net files differ
(analysis/build_geometry_sweep_nets_span700.py's span=700 builds, all new;
none of the span=400 sweep's nets are reusable here since C2's absolute
position differs for every r at a different span).

Deliberately bypasses compare.py -- same reason geometry_sweep.py gives.

    python -m analysis.build_geometry_sweep_nets_span700   # once
    python -m analysis.geometry_sweep_span700
"""
import os
import sys
...
OUT_CSV = os.path.join(REPO, "analysis", "geometry_sweep_span700.csv")
...
RATIO_NETS = {
    0.50: "corridor_geom700_350.net.xml",
    0.55: "corridor_geom700_385.net.xml",
    0.60: "corridor_geom700_420.net.xml",
    0.65: "corridor_geom700_455.net.xml",
    0.70: "corridor_geom700_490.net.xml",
    0.75: "corridor_geom700_525.net.xml",
    0.80: "corridor_geom700_560.net.xml",
    0.90: "corridor_geom700_630.net.xml",
}
...
def report(df: pd.DataFrame) -> None:
    print(f"\n=== {SCENARIO}, mg{MIN_GREEN}, span=700: delay/trip vs ratio ===")
